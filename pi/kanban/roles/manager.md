...
6. if that leaves no active worker and there is a single clear next task, launch it in the same pass
7. record the result in the pane output
8. stop and wait only if there is no clear immediate follow-on action
