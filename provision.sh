...
function install_osx_gcc() {
  ...
  if [ "$osx_version" =~ ^10\.6 ]; then
    installbuddy "https://github.com/downloads/kennethreitz/osx-gcc-installer/GCC-10.6.pkg"
  elif [ "$osx_version" =~ ^10\.7 ]; then
    installbuddy "https://github.com/downloads/kennethreitz/osx-gcc-installer/GCC-10.7-v2.pkg"
  fi
}
