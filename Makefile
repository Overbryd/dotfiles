...
PI_CONFIG_SOURCES := $(filter-out pi/settings.json,$(wildcard pi/*.json))
...
~/.config/%: ~/.config
	cd ~ && test -h ~/.config/$(notdir $@) || ln -svf $(DOTFILES_ROOT)/.config/$(notdir $@) .config/$(notdir $@)
